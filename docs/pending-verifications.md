...
| PV-007 | **QV-041 live news ingestion — hourly Beat cadence.** The multi-provider `ingest_news` job (NewsAPI/GNews/Marketaux/Finnhub behind `INewsProvider`) is built, network-free tested, and **all 4 providers live-verified (2026-07-11):** a live run ingested real India-financial articles (Times of India, BusinessLine, Livemint, Economic Times, Moneycontrol, Reuters/CNBC + Marketaux with entity+sentiment), `NewsIngested` emitted, cross-provider dedup confirmed, **0 provider failures**. **Remaining:** wire the hourly `crontab(minute=0)` entry into `beat_schedule` (deferred like prices/macro — no live scheduler in dev). | Beat cadence is deferred by convention (data jobs stay off live Beat until a scheduler/staging). The pull itself is fully verified here. | Add the hourly entry to `beat_schedule` when a live scheduler runs (staging). Live-run recipe (re-runnable here): `python -c "from datetime import datetime, UTC; from quantvista.jobs.news import _run_news, run_key; n=datetime.now(UTC); print(_run_news(n, run_key('news', n.strftime('%Y-%m-%dT%H'))).status.value)"` → `jobs_runs` succeeded + `SELECT count(*) FROM news` grows + dedup on re-run. | **Before the news pipeline runs unattended in staging/prod.** (All 4 sources are already live-verified; sentiment QV-044 / per-stock feed QV-043 are unblocked.) | ⏳ OPEN (pull verified for all 4 providers; only Beat cadence remains) |
