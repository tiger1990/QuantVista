...
- `frontend/src/features/backtests/BacktestList.test.tsx` (new — clickable history rows)

### Known data limitation (dev box, not a code defect)

`daily_prices` currently spans **2025-06-02 → 2026-07-24** (286 sessions). A short window like 17–31 Jul 2026 has only **6 sessions** and, at monthly cadence, yields a degenerate run (≈0 rebalances, near-empty equity curve). For a meaningful tearsheet use a range inside the ingested span (e.g. 2025-07-01 → 2026-07-24). The tearsheet renders empty/degenerate runs gracefully (AC-5), so this is a data-coverage note, not a bug.
