...
- **`reproducibility_hash` was unexplained (fixed):** the footer showed a bare `repro <hex>…`. Relabelled `recipe …` with a title explaining it is SHA-256 of the spec + `model_version`/`weights_version`, **and that it does not fingerprint the underlying price/score data** — the same recipe can yield different numbers when the data moves.
